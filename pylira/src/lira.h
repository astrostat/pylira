...
void read_data_or_image(
    int image, /* 1 to read in an image, 0 to read in data */
    cntType *cnt) {
  ...
  for (i = 0; i < cnt->nrow; i++) {
    for (j = 0; j < cnt->ncol; j++) {
      ...
      if (image)
        cnt->img[i][j] = (double)ftemp;
      else
        cnt->data[i][j] = (double)ftemp;
    } /* column loop */
  }   /* row loop */
  ...
  if (verbose > 3) {
    Rprintf("%s ", cnt->name);
    if (image)
      print_mat("Image", cnt->img, cnt->nrow, cnt->ncol);
    else
      print_mat("Data", cnt->data, cnt->nrow, cnt->ncol);
  } /* verbose */
}
  ...
  if (mrf->is_hi_res) {
    ...
    for (i = 0; i < cnt->nrow; i++) {
      for (j = 0; j < cnt->ncol; j++) {
        ...
        if (ftemp <= 0) {
          REprintf("Error in reading element (%d, %d) of MRF HIGH RESOLUTION "
                   "DATA.\n",
                   i, j);
    ...
    for (i = 0; i < cnt->nrow; i++) {
      for (j = 0; j < cnt->ncol; j++) {
        ...
        for (k = 0; k <= 2; k++) {
          for (l = 0; l <= 2; l++) {
            nbr = mrf->hi_res[wrap(i + k - 1, cnt->nrow)]
                             [wrap(j + l - 1, cnt->ncol)];
            mrf->wts[i][j][k][l] =
                (mrf->hi_res[i][j] - nbr) * (mrf->hi_res[i][j] - nbr);
            mrf->wts[i][j][k][l] =
                mrf->scl_prec /* max_wt keeps wts from infty! */
                / (mrf->wts[i][j][k][l] + 1 / mrf->max_wt);
            mrf->mn_wt += mrf->wts[i][j][k][l];
            mrf->n_wts += 1;
            if (k * l != 1)
              mrf->prec[i][j] += mrf->wts[i][j][k][l];
    ...
    for (j = 0; j < src->ncol; j++) {
      for (k = -1; k < 2; k++) {
        for (l = -1; l < 2; l++) {
          ...
          if ((i + k >= 0) && (i + k < src->nrow) && (j + l >= 0) &&
              (j + l < src->ncol) && (k * k + l * l > 0)) {
...
void redistribute_Counts(psfType *psf, cntType *obs, cntType *deblur,
          ...
          /********** if too close to the edge **************/
            ...
            if (cont->wrap) {
              psf->inv[k][l] = /* Bayes Theorem on staroids. */
                  ...
                  psf->mat[psf->nrow - k - 1][psf->ncol - l - 1];
              sum += deblur->img[wrap(i - psf->D + k, obs->nrow)]
                                [wrap(j - psf->R + l, obs->ncol)] *
                     psf->mat[psf->nrow - k - 1][psf->ncol - l - 1];
            } /* if wrap */
            else
  ...
  if (verbose > 5)
    print_mat("Redistributed Counts", deblur->data, deblur->nrow, deblur->ncol);
      ...
      for (k = -1; k < 2; k++) { /* COMPUTE THE MRF MEAN FOR PIXEL (i, j) */
        for (l = -1; l < 2; l++) {
          ...
          if ((i + k >= 0) && (i + k < src->nrow) && (j + l >= 0) &&
              (j + l < src->ncol) && (k * k + l * l > 0)) {
            mrf->mean[i][j] +=
                log(src->img[i + k][j + l]) * mrf->wts[i][j][k + 1][l + 1];
          } /* if an adjacent neighbor */
          else if (k * k + l * l > 0 && cont->wrap) {
            mrf->mean[i][j] +=
                log(src->img[wrap(i + k, src->nrow)][wrap(j + l, src->ncol)]) *
...
update_alpha_ms_MH(double prop_mean,      /* the mean of the proposal dist'n */
  ...
  if ((prop_mean * sqrt(prop_sd)) < 1e-10)
    ...
    prop_sd = MH_sd_inflate / sqrt(prop_sd);

  for (i = 0; i < MH_iter; i++) {
    proposal = rlnorm(lg_prop_mn, prop_sd); /* log normal proposal dist'n */
    log_ratio = lpost_lalpha(proposal, ms, level, dim) -
...
void update_scale_model(scalemodelType *scalemodel, expmapType *expmap,
  ...
  if (cont->em) { /* if EM compute mode */
    ...
    scalemodel->scale = rgamma(total_cnt + scalemodel->scale_pr,
                               1 / (total_exp + scalemodel->scale_exp));
} /* update_scale_model */
...
void print_param_file_header(FILE *param_file, controlType *cont,
  ...
  for (i = 0; i < ms->power; i++) {
    ...
    if (i == 0)
      fprintf(param_file, " (Full Data)\n");
    else if (i == ms->power - 1)
      fprintf(param_file, "  (In the 2x2 table)\n");
    else
      fprintf(param_file, "\n");
  } /* loop over ms.al */
